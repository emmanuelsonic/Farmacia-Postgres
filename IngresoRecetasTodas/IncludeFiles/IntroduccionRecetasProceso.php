...
$proceso->EliminarReceta($IdHistorialClinico,$IdPersonal,$IdReceta,$IdArea,$_SESSION["IdEstablecimiento"],$IdModalidad);
...
//	INTRODUCCION DE MEDICAMENTO ENTREGADOS
    ...
    $IdMedicinaRecetada=$proceso->IntroducirMedicinaPorReceta($IdReceta,$IdMedicina,$Cantidad,$Dosis,
                                                                $Satisfecha,$Fecha,$IdEstablecimiento,$IdModalidad);
...
case 13:
	/*MOSTRAR SUBESPECIALIDADES O SERVICIO ORIGEN DE RECETA*/
		...
		$query="select msse.IdSubServicioxEstablecimiento,NombreSubServicio, IdServicio  as Ubicacion
...
case 14:
	...
	if(isset($_GET["IdArea"])){
		...
		if($IdReceta=='' or $IdReceta==NULL){
			...
			echo $proceso->ActualizarArea($IdArea,$IdReceta,$IdFarmacia,$Fecha,$_SESSION["IdEstablecimiento"],$IdModalidad);
		}
	...
	if(isset($_GET["IdMedico"])){
		...
		if($IdHistorialClinico=='' or $IdHistorialClinico==NULL){
			...
			$proceso->ActualizarMedico($IdHistorialClinico,$IdMedico,$_SESSION["IdEstablecimiento"],$IdModalidad);
		}
	...
	if(isset($_GET["IdSubServicio"])){
		...
		if($IdHistorialClinico=='' or $IdHistorialClinico==NULL){
			...
			$proceso->ActualizarSubServicio($IdHistorialClinico,$IdSubServicio,$_SESSION["IdEstablecimiento"],$IdModalidad);
		}
...
case 17:
	//Listado de Areas
	$IdArea=$_GET["IdArea"];
	echo $proceso->AreaOrigen($IdArea,$_SESSION["IdEstablecimiento"]);
