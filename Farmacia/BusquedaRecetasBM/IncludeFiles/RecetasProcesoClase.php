    ...
    function ObtenerMedicinaIntroducida($IdReceta,$IdEstablecimiento,$IdModalidad) {
        $querySelect = "select farm_recetas.IdHistorialClinico, FechaConsulta, farm_medicinarecetada.*,farm_catalogoproductos.Nombre,
                        farm_catalogoproductos.Concentracion,IdArea,FormaFarmaceutica,Presentacion,IdFarmacia,
                                (
                                    select group_concat(Lote SEPARATOR '-') 
                                    from farm_lotes fl 
                                    inner join farm_medicinadespachada fmd 
                                    on fl.IdLote=fmd.IdLote 
                                    where fmd.IdMedicinaRecetada=farm_medicinarecetada.IdMedicinaRecetada
                                ) as Lotes 
						from farm_medicinarecetada
...
//actualizaicon de area
    ...
    function ObtenerArea() {
        $query = "select distinct mafxe.IdArea, Area, case IdFarmacia when 1 then '(Farm. Central)' when 2 then '(Con. Ext.)' when 3 then '(Emergencia)' end as Origen
			from mnt_areafarmacia maf
                        inner join mnt_areafarmaciaxestablecimiento mafxe
                        on mafxe.IdArea = maf.IdArea
			where maf.IdArea != 7 and maf.IdArea != 12
...
//**************	FIN DE MANEJO DE LOTES Y EXISTENCIAS	**********************************
    ...
    function ObtenerAreaReceta($IdReceta,$IdEstablecimiento,$IdModalidad) {
        $SQL = "select IdArea from farm_recetas 
                where IdReceta=" . $IdReceta."
                and IdEstablecimiento=$IdEstablecimiento
                and IdModalidad=$IdModalidad";
	...
	and IdArea=" . $IdArea;
        ...
        if ($row = mysql_fetch_array($this->ValorDivisor($IdMedicina,$IdEstablecimiento,$IdModalidad))) {
