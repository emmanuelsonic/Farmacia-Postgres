...
class Classquery{
	...
	function ObtenerQuery($Bandera,$IdArea,$FechaAtras,$FechaAdelante,$q){
	switch($Bandera){
...
year(curdate()) as year2,concat_ws('-',day(farm_recetas.Fecha),month(farm_recetas.Fecha),year(farm_recetas.Fecha))as FechaPreparada
...
and farm_recetas.Fecha not in(select farm_recetas.Fecha
								from farm_recetas
								where  (farm_recetas.IdEstado='N' OR farm_recetas.IdEstado='L' OR farm_recetas.IdEstado='RN' OR farm_recetas.IdEstado='RL')
...
function ObtenerQueryTotal($Bandera,$IdArea,$FechaAtras,$FechaAdelante,$q){
switch($Bandera){
case 1:
 $sqlStrAux = "select count(distinct farm_recetas.IdReceta) as total
from sec_historial_clinico
inner join mnt_expediente
on mnt_expediente.IdNumeroExp=sec_historial_clinico.IdNumeroExp
inner join farm_recetas
