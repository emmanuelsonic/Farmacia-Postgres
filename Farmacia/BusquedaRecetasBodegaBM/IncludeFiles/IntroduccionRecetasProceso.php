...
//REPETITIVAS
        ...
        case 12:
            ...
            if ($CodigoFarmacia != '') {
                $resp = $proceso->ObtenerDatosMedico($CodigoFarmacia, $IdEstablecimiento, $IdModalidad);
        ...
        case 15:
			...
			<option value='0'>[Seleccione ...]</option>";
                    $resp = $proceso->ObtenerArea($_SESSION["TipoFarmacia"], $IdEstablecimiento, $IdModalidad);
                    while ($row = mysql_fetch_array($resp)) {
	   ...
	   <input type='button' id='Cambiar3' name='Cambiar3' value='Corregir' onClick='CorregirArea();'>
                ...
                case 'NombreAreaOrigen':
                    $IdReceta = $_GET["IdReceta"];
                    $IdAreaOriginal = $proceso->ObtenerAreaOrigenReceta($IdReceta, $IdEstablecimiento, $IdModalidad);
                    $Tools = "<table>
			<tr class='FONDO2'>
			<td>
			<select id='IdAreaOrigen2' name='IdAreaOrigen2' onChange='PegarIdAreaOrigen(this.value);'>
			<option value='0'>[Seleccione ...]</option>";
                    $resp = $proceso->ObtenerAreaOrigen($_SESSION["TipoFarmacia"], $IdEstablecimiento, $IdModalidad);
                    while ($row = mysql_fetch_array($resp)) {
                        $Tools.="<option value='" . $row[0] . "'>" . $row[1] . " <strong><em>" . $row[2] . "</em></strong></option>";
                    }
