...
//html { font: 100%/2.5 Arial, Helvetica, sans-serif; }
        ...
        if ($IdEmpleado != '0') {
            $t = mysql_fetch_array(mysql_query("select NombreEmpleado from mnt_empleados where IdEmpleado='$IdEmpleado'"));
            $NombreEmpleado = "Medico: <strong>" . $t[0] . "</strong><br>";
        } else {
            $NombreEmpleado = "Medico: <strong>GENERAL</strong> <br>";
        }

        /*         * ********************INFORMACION DE REPORTE********************************* */
        if (isset($_GET["IdSubServicio"])) {
            $IdSubEspecialidad = $_GET["IdSubServicio"]; //IdSubServicioxEstablecimiento
        } else {
            $IdSubEspecialidad = 0;
        }
        ...
        if ($IdSubEspecialidad != 0) {
            $t = mysql_fetch_array(mysql_query("select msse.IdSubServicioxEstablecimiento,NombreServicio,NombreSubServicio 
				...
				and msse.IdSubServicioxEstablecimiento='$IdSubEspecialidad'
                                and mse.IdEstablecimiento=$IdEstablecimiento
                                and mse.IdModalidad=$IdModalidad
                                "));

            $Especialidad_Servicio = "Especialidad / Servicio: <strong>[ " . $t[1] . " ] " . $t[2] . "</strong><br>";
        } else {
            $Especialidad_Servicio = "Especialidad / Servicio: <strong>GENERAL</strong><br>";
        }
...
//******************************* QUERIES Y RECORRIDOS
        $respServicios = Servicios($IdSubEspecialidad, $IdGrupoTerapeutico, $IdMedicina, $FechaInicio, $FechaFin, $IdEmpleado, $IdFarmacia, $IdEstablecimiento, $IdModalidad);
	...
	<td colspan="11" align="center" style="background:#666666;">
		<strong><h2>[' . $Ubicacion . '] ' . $NombreSubEspecialidad . '</h2></strong>
	</td>
	</tr>';


                $nombreTera = NombreTera($IdGrupoTerapeutico, $IdSubEspecialidad, $FechaInicio, $FechaFin, $IdEmpleado, $IdFarmacia,$IdEstablecimiento,$IdModalidad);
		...
		&nbsp;<strong>' . $NombreTerapeutico . '</strong></td>
			...
			<tr class="FONDO2">
			  ...
			  <th width="136" scope="col">Monto[$]</th>
                        ...
                        $resp1 = QueryExterna($IdTerapeutico, $IdMedicina, $IdSubEspecialidad, $FechaInicio, $FechaFin, $IdEmpleado, $IdFarmacia,$IdEstablecimiento,$IdModalidad);
                        while ($row = mysql_fetch_array($resp1)) {
                            ...
                            if ($row2 = mysql_fetch_array($respuesta)) { /* verificacion de datos */
                                ...
                                $sat = ObtenerRecetasSatisfechas($IdReceta, $Medicina, $FechaInicio, $FechaFin, $IdSubEspecialidad, 0, $IdEmpleado, $IdFarmacia,$IdEstablecimiento,$IdModalidad);
                                $insat = ObtenerRecetasInsatisfechas($IdReceta, $Medicina, $FechaInicio, $FechaFin, $IdSubEspecialidad, 0, $IdEmpleado, $IdFarmacia,$IdEstablecimiento,$IdModalidad);
                                $Nrecetas = $sat + $insat;
                                //***********
                                //***********
                                /*
                                ...
                                $respSum = SumatoriaMedicamento($Medicina, $IdSubEspecialidad, $FechaInicio, $FechaFin, $IdEmpleado, $IdFarmacia,$IdEstablecimiento,$IdModalidad);
