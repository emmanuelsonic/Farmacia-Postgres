...
//Intro Receta Nueva
    ...
    function EliminarReceta($IdHistorialClinico, $IdPersonal, $IdReceta) {
        ...
        $IdArea = mysql_fetch_array(mysql_query($querySelect1));
        if ($IdArea[0] == 2) {
            /*             * ************************************************* */
            ...
            $resp2 = mysql_query($querySelect2);
        ...
        while ($row = mysql_fetch_array($resp2)) {
            ...
            $respuesta = mysql_fetch_array(mysql_query($querySelect2));
            if ($respuesta != NULL) {
                mysql_query($queryDeleteMedicina2);
            }
            mysql_query($queryDeleteReceta2);
        }//fin de while
...
//Introducir Medicina Receta
    ...
    function ObtenerMedicinaIntroducida($IdReceta, $IdEstablecimiento, $IdModalidad) {
        $querySelect = "select farm_recetas.IdHistorialClinico, FechaConsulta, farm_medicinarecetada.*,farm_catalogoproductos.Nombre,farm_catalogoproductos.Concentracion,IdArea,FormaFarmaceutica,Presentacion,IdFarmacia
						...
						where (farm_medicinarecetada.IdReceta like '$IdReceta' or CorrelativoAnual='$IdReceta')
...
//Obtener Medicina Introducida
    ...
    function ObtenerDatosGenerales($IdReceta, $IdEstablecimiento, $IdModalidad) {
        $query = "select farm_recetas.IdArea,(select Area from mnt_areafarmacia where IdArea=farm_recetas.IdArea) as Area, sec_historial_clinico.IdNumeroExp,
                                NombreEmpleado,NombreSubServicio, 
                               ( select NombreServicio 
                                    from mnt_servicio ms 
                                    inner join mnt_servicioxestablecimiento mse 
                                    on ms.IdServicio=mse.IdServicio 
                                    inner join mnt_subservicioxestablecimiento msse 
                                    on msse.IdServicioxEstablecimiento = mse.IdServicioxEstablecimiento 
                                ...
                                case farm_recetas.IdFarmacia when 1 then 'Central' 
                                                             when 2 then 'Con. Externa' 
                                                             when 3 then 'Emergencias' 
                                                             when 4 then 'Bodega' end as NombreFarmacia,
                                IdAreaOrigen,(select Area from mnt_areafarmacia where IdArea=farm_recetas.IdAreaOrigen) as AreaOrigen
...
//UpdateDosis
    ...
    function UpdateCantidad($IdMedicinaRecetada, $Cantidad) {
        ...
        $rowCantidad = mysql_fetch_array(mysql_query($querySelect1));
        $querySelect2 = "select IdArea
					from farm_recetas
					inner join farm_medicinarecetada
					on farm_recetas.IdReceta=farm_medicinarecetada.IdReceta
					where IdMedicinaRecetada=" . $IdMedicinaRecetada;
        $IdArea = mysql_fetch_array(mysql_query($querySelect2));
        ...
        if ($IdArea[0] == 2) {
            ...
            $resp1 = mysql_fetch_array(mysql_query($querySelect3));
            ...
            if ($rowCantidad[1] == 'S') {
                if ($rowCantidad[0] < $Cantidad) {
                    ...
                    if ($IdLote2 != '' and $IdLote2 != NULL and $IdLote2 != '0') {
                        ...
                        $rowExistencia = mysql_fetch_array(mysql_query($querySelect4));
                        ...
                        if ($rowExistencia[0] != 0) {
                            ...
                            mysql_query($UpdateCantidadLote2);
                        }
                        /*                         * ******************************************************************************************** */
                    } else {
                        ...
                        if ($rowExistencia[0] < $Diferencia) {
                            ...
                            if ($IdLote2 != '' and $IdLote2 != '0' and $IdLote2 != NULL) {
                                ...
                                $rowExistencia2 = mysql_fetch_array(mysql_query($SelectExistencia2));
                    ...
                    $rowCantidad = mysql_fetch_array(mysql_query($queryCantidad));
                    ...
                    $selectLotes = "select CantidadLote1,Lote1,CantidadLote2,Lote2
								from farm_medicinarecetada
								where IdMedicinaRecetada=" . $IdMedicinaRecetada;
                    $respLotes = mysql_fetch_array(mysql_query($selectLotes));
                    ...
                    if ($IdLote2 != '0' and $IdLote2 != '' and $IdLote2 != NULL) {
                        ...
                        if ($CantidadLote2 <= $Diferencia) {
                            ...
                            $SelectExistencia2 = "select Existencia from farm_medicinaexistenciaxarea where IdLote=" . $IdLote2;
                            $rowExistencia1 = mysql_fetch_array(mysql_query($SelectExistencia1));
                            $rowExistencia2 = mysql_fetch_array(mysql_query($SelectExistencia2));
                        ...
                        mysql_query($UpdateCantidadLote1);
                    }
                }
            }//IdEstado =='S'
        } else {
    ...
    function UpdateMedicinaRecetada($IdMedicinaRecetada, $Estado, $IdMedicina) {
        if ($Estado == 'I') {
            ...
            if ($IdArea[0] == 2) {
                ...
                if ($IdLote2 != '' and $IdLote2 != NULL and $IdLote2 != '0') {
                    $querySelect4 = "select Existencia from farm_medicinaexistenciaxarea where IdLote=" . $IdLote2;
                    $resp4 = mysql_fetch_array(mysql_query($querySelect4));
            ...
            $querySelectArea = "select IdArea, farm_medicinarecetada.IdEstado 
									...
									on farm_medicinarecetada.IdReceta=farm_Recetas.IdReceta
									where IdMedicinaRecetada=" . $IdMedicinaRecetada;
            $IdArea = mysql_fetch_array(mysql_query($querySelectArea));
            ...
            if ($IdArea[0] == 2) {
                ...
                $querySelect1 = "select Existencia, farm_lotes.IdLote
							...
							order by FechaVencimiento
							limit 1";
                $resp1 = mysql_fetch_array(mysql_query($querySelect1));
                $IdLote = $resp1["IdLote"];
                ...
                if ($resp1["Existencia"] < $Cantidad) {
                    ...
                    $resp2 = mysql_fetch_array(mysql_query($querySelect1));
                ...
                if ($resp["Existencia"] < $Cantidad) {
                    ...
                    /*                     * *******	MANIPULACION DEL SEGUNDO LOTE	******** */
                    $resp2 = mysql_fetch_array(mysql_query($querySelect));
...
//CodigoFarmacia
    ...
    function ObtenerDatosMedico($CodigoFarmacia, $IdEstablecimiento, $IdModalidad) {
...
//ObtenerDatosMedico
    ...
    function ActualizarArea($IdArea, $IdReceta, $IdFarmacia, $IdEstablecimiento, $IdModalidad) {
        ...
        /* $queryH = "select IdHistorialClinico 
          from farm_recetas where IdReceta='$IdReceta'
          and IdEstablecimiento=$IdEstablecimiento
          and IdModalidad=$IdModalidad";
...
//actualizaicon de area
    ...
    function ActualizarEspecialidad($IdHistorialClinico, $IdSubServicio, $IdEstablecimiento, $IdModalidad) {
        if ($IdSubServicio != '') {
            $query = "update sec_historial_clinico set IdSubServicio='$IdSubServicio' 
                        where IdHistorialClinico='$IdHistorialClinico'
                        and IdEstablecimiento=$IdEstablecimiento
                        and IdModalidad=$IdModalidad";
            mysql_query($query);
        }
        $query2 = "select mssxe.IdSubServicio,NombreSubServicio, NombreServicio as Ubicacion
                        ...
                        on shc.IdSubServicio=mssxe.IdSubServicio
                        where IdHistorialClinico='$IdHistorialClinico'
                        ...
                        and mssxe.CodigoFarmacia='$Codigo'";
        $resp = mysql_fetch_array(mysql_query($query2));
    ...
    function ObtenerArea($TipoFarmacia, $IdEstablecimiento, $IdModalidad) {
        if ($TipoFarmacia == 1) {
            $comp = " and mafxe.IdFarmacia <> 4";
        } else {
            $comp = "";
        }
        $query = "select mafxe.IdArea, Area, 
                    case IdFarmacia when 1 then '(Farm. Central)' 
                                    when 2 then '(Con. Ext.)' 
                                    when 3 then '(Emergencia)' end as Origen
			from mnt_areafarmacia maf
                        inner join mnt_areafarmaciaxestablecimiento mafxe
                        on mafxe.IdArea= maf.IdArea
			where mafxe.IdArea != 7
    ...
    function ObtenerAreaOrigen($TipoFarmacia, $IdEstablecimiento, $IdModalidad) {
        if ($TipoFarmacia == 1) {
            $comp = " and mafxe.IdFarmacia <> 4";
        } else {
            $comp = "";
    ...
    function CambiarFecha($IdReceta, $FechaNueva, $IdHistorialClinico, $IdEstablecimiento, $IdModalidad) {
        $query = "update farm_recetas set Fecha='$FechaNueva' 
                    where IdReceta=$IdReceta
                    and IdEstablecimiento=$IdEstablecimiento
                    and IdModalidad=$IdModalidad";
        $query2 = "update farm_medicinarecetada set FechaEntrega='$FechaNueva' 
                    where IdReceta=$IdReceta
                    and IdEstablecimiento=$IdEstablecimiento
                    and IdModalidad=$IdModalidad";
        $query3 = "update sec_historial_clinico set FechaConsulta='$FechaNueva' 
                    where IdHistorialClinico=$IdHistorialClinico
                    and IdEstablecimiento=$IdEstablecimiento
                    and IdModalidad=$IdModalidad";
        mysql_query($query);
    ...
    function CambiarExpediente($IdNumeroExp, $IdHistorialClinico, $IdEstablecimiento, $IdModalidad) {
...
//aumento de existencias por eliminacion de recetas...
    ...
    function ActualizacionInventarioCantidad($IdMedicinaRecetada, $NuevaCantidad, $IdArea) {
        ...
        if ($NuevaCantidad < $datos["Cantidad"]) {
			...
			inner join farm_entragamedicamento
...
//**************	FIN DE MANEJO DE LOTES Y EXISTENCIAS	**********************************
    ...
    function ObtenerAreaOrigenReceta($IdReceta, $IdEstablecimiento, $IdModalidad) {
        $SQL = "select IdAreaOrigen 
                from farm_recetas 
                where IdReceta=$IdReceta
                and IdEstablecimiento=$IdEstablecimiento
                and IdModalidad=$IdModalidad";
