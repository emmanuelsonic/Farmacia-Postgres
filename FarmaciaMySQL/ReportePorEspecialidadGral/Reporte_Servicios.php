...
if(isset($_GET["IdFarmacia"])){$IdFarmacia=$_GET["IdFarmacia"];}else{$IdFarmacia=0;}
if(isset($_GET["IdSubEspecialidad"])){$IdSubEspecialidad=$_GET["IdSubEspecialidad"];}else{$IdSubEspecialidad=0;} //IdSubServicioxEstablecimiento
...
if(isset($_GET["IdFarmacia"])){$IdFarmaciaN=$_GET["IdFarmacia"];}else{$IdFarmaciaN=0;}
...
<tr class="MYTABLE"><td align="right" colspan="11">Fecha de Emisi&oacute;n: '.$DateNow=date("d-m-Y").'</td>
	...
	if($grupos=mysql_fetch_array($nombreTera)){
		...
		$respuesta=ObtenerReporteGrupoTerapeutico($IdTerapeutico,$Medicina,$FechaInicio,$FechaFin,$IdSubEspecialidad,$IdEstablecimiento,$IdModalidad);
		    ...
		    if($row2=mysql_fetch_array($respuesta)){ /* verificacion de datos */
		        ...
		        $sat=ObtenerRecetasSatisfechas($Medicina,$FechaInicio,$FechaFin,$IdSubEspecialidad,0,0,$IdFarmacia,$IdEstablecimiento,$IdModalidad);
		        $insat=ObtenerRecetasInsatisfechas($Medicina,$FechaInicio,$FechaFin,$IdSubEspecialidad,0,0,$IdFarmacia,$IdEstablecimiento,$IdModalidad);
		        	$Nrecetas=$sat+$insat; 
		        //***********
    ...
    $reporte.='<tr class="FONDO2" style="background:#CCCCCC;">
	  ...
	  <td align="right">&nbsp;</td>
	  <td align="right">&nbsp;</td>
	  <td align="right"><strong>'.number_format($TotalEspecialidad,3,'.',',').'</strong></td>
    </tr>';
