...
window.opener.PegarEspecialidad(IdSubServicio,CodigoFarmacia);
    ...
    <?php 
		...
		if(isset($page)){
				...
				if($row["Ubicacion"]=='INSUMO' or $row["Ubicacion"]=="CONEXT"){$Ubicacion=$row["Ubicacion"]." -> ";}else{$Ubicacion="HOSPIT. -> ";}
                                if($row["Ubicacion"]=='CONBMG'){$Ubicacion="CONBMG -> ";}
echo "\t\t<tr class=\"row$r\">
...
<td align=\"left\"><a href=\"#\" onclick=\"javascript:UbicarSubServicio(".$row['IdSubServicioxEstablecimiento'].",'".htmlentities($row['CodigoFarmacia'])."')\">".$Ubicacion."".htmlentities($row['NombreSubServicio'])."</a></td>
</tr>";
	}//if
