...
//Introducir Medicina Receta
    ...
    function ObtenerMedicinaIntroducida($IdReceta, $IdEstablecimiento, $IdModalidad) {
        $querySelect = "select farm_recetas.IdHistorialClinico, FechaConsulta, farm_medicinarecetada.*,farm_catalogoproductos.Nombre,
                        farm_catalogoproductos.Concentracion,IdArea,FormaFarmaceutica,Presentacion,IdFarmacia
						from farm_medicinarecetada
...
//Obtener Medicina Introducida
    ...
    function ObtenerDatosGenerales($IdReceta, $IdEstablecimiento, $IdModalidad) {
        $query = "select farm_recetas.IdArea,(select Area from mnt_areafarmacia where IdArea=farm_recetas.IdArea) as Area, 
...
//actualizaicon de area
    ...
    function ObtenerIdFarmacia($IdArea, $IdEstablecimiento, $IdModalidad) {
        $query = "select maf.IdFarmacia 
				from mnt_areafarmacia maf
                                inner join mnt_areafarmaciaxestablecimiento mafxe
                                on mafxe.IdArea = maf.IdArea
				where mafxe.IdArea=" . $IdArea . "
                                ";
        $resp = mysql_fetch_array(mysql_query($query));
        return($resp[0]);
    }
    ...
    function ActualizarMedico($IdHistorialClinico, $IdMedico, $IdEstablecimiento, $IdModalidad) {
        ...
        $query2 = "select NombreEmpleado from mnt_empleados 
                            inner join sec_historial_clinico shc 
                            on shc.IdEmpleado=mnt_empleados.IdEmpleado 
                            where IdHistorialClinico=" . $IdHistorialClinico . "
    ...
    function ActualizarEspecialidad($IdHistorialClinico, $IdSubServicio, $Codigo, $IdEstablecimiento, $IdModalidad) {
        ...
        $query2 = "select mssxe.IdSubServicioxEstablecimiento,NombreSubServicio, NombreServicio as Ubicacion
    ...
    function ObtenerArea($TipoFarmacia) {
        if ($TipoFarmacia == 1) {
            $comp = " and IdFarmacia <> 4";
        } else {
            $comp = "";
        }
    ...
    function ObtenerAreaOrigen($TipoFarmacia) {
        if ($TipoFarmacia == 1) {
            $comp = " and IdFarmacia <> 4";
        } else {
            $comp = "";
...
//**************	FIN DE MANEJO DE LOTES Y EXISTENCIAS	**********************************
	...
	where fem.IdMedicina=" . $IdMedicina . "
